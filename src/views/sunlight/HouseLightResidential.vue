...
export default class SunOpen {
  ...
  updateSouthSunShine(houseSunShineDataList) {
    for (var i = 0; i < houseSunShineDataList.length; i++) {
      let houseSunShineData = houseSunShineDataList[i];
      let swallDataList = houseSunShineData.swallDataList
      for (var southWallIndex = 0; southWallIndex < swallDataList.length; southWallIndex++) {
        let southWallData = swallDataList[southWallIndex]
        let meshKey = houseSunShineData.buildingNumber + "_" + houseSunShineData.unit + "_" + houseSunShineData.floorNumber + "_" + houseSunShineData.room + "_" + southWallData.swallName
        let southMesh = this.southWallMeshMap.get(meshKey)
        this.updateSouthWallColor(southMesh, southWallData.sunshineTime);
      }
    }
  }
  ...
  updateSouthWallColor(mesh, sunTime) {
    let color = 0xffffff;
    if (sunTime > 480) {
      color = 0xf56411
    } else if (sunTime > 420 && sunTime <= 480) {
      color = 0xf67328
    } else if (sunTime > 360 && sunTime <= 420) {
      color = 0xf78341
    } else if (sunTime > 300 && sunTime <= 360) {
      color = 0xf89258
    } else if (sunTime > 240 && sunTime <= 300) {
      color = 0xf9a270
    } else if (sunTime > 180 && sunTime <= 240) {
      color = 0xfab188
    } else if (sunTime > 120 && sunTime <= 180) {
      color = 0xfbc1a0
    } else if (sunTime > 60 && sunTime <= 120) {
      color = 0xfcd0b7
    } else if (sunTime > 0 && sunTime <= 60) {
      color = 0xfde0cf
    } else if (sunTime == 0) {
      color = 0xfeefe7
    }
